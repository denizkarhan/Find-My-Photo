...
from PyQt5.QtGui import QImage, QPalette, QBrush, QPixmap, QFont, QPixmap, QImage, QPalette
from IPython.utils.path import ensure_dir_exists
import sys, os, torch, cv2, numpy, face_recognition, traceback
import shutil
...
class Window(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.buton1.setGeometry(QtCore.QRect(150, 200, 200, 75))
        layout = QVBoxLayout()
    ...
    def filtre(self):
        size = 960
        resizedImages(self.inputDirectory, size)
        if "" in [self.inputDirectory, self.outputDirectory]:
            print("Lütfen girdi ve cikti dosyasini giriniz!")
            return

        self.inputDirectory = f"{self.inputDirectory}\_RESIZED_{str(size)}"
...
class multiFaceProcessor:
    def __init__(self, inputPath, category, MainUi, inputImages="allFaces"):
        ...
        # Window.ui.addPrompt("-) Buradayim be buradayim!\n")
        self.faces_folder_path = os.path.join(inputPath, inputImages)

        folder_path = Path(self.faces_folder_path)
        if folder_path.exists():
            if Window.showPopup(self.MainUi, "Eski verileri silmek ister misiniz?") == 1:
        ...
        # folders = [d for d in os.listdir(inputPath) if os.path.isdir(os.path.join(inputPath, d))]
        self.inputPath = os.path.join(inputPath, folder)

        try:
            # os.system(f"mkdir {self.faces_folder_path}")
...
def compareStart(inputPath, outputPath, personCount, faceDistance, MainUi):
    ...
    face_processor.organize_folders()
